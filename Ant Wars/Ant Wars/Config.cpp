...
int GConfig::s_BlockWidth = 128;
int GConfig::s_BlockHeight = 128;
int GConfig::s_BlockAtlasWidth = 128; 
int GConfig::s_BlockAtlasHeight = 128; 
float GConfig::s_Gravity = 9.81f;
float GConfig::s_PlayerJump = 1.0f;
int GConfig::s_PlayerSrcRectWidth = 96;
int GConfig::s_PlayerSrcRectHeight = 89;
int GConfig::s_BulletSpeed = 250;
...
void GConfig::LoadConfig(const char* _pFile)
{
	...
	while (text.length() > 0)
	{
		...
		// if variable is bullet speed set value
			...
			s_BulletSpeed = (int)(stoi(value) * 0.001f);
