...
int CEngine::Init()
{
	...
	m_pGGM = new GGameManager();

	// error if game manager nullptr
	if (!m_pGGM)
	{
		LOG_ERROR("Game Manager not initialized", 204);
	}
...
void CEngine::ChangeScene(CScene* _pScene)
{
	...
	if (m_pScene)
	{
		...
		// clean ui objects [added by Vladi]
...
void CEngine::Update()
{
	...
	// update game manager
	m_pGGM->Update(m_pTime->GetDeltaTime());
